...
調査日: 2026-09-07。ステータス: 提案。今回の成果物は調査と計画であり、アプリの実装変更・有料 API の動作検証・デプロイは含まない。
...
- 個人利用・ログイン不要を維持する。費用は AI だけでなく Maps / Web 検索 / インフラを含めて把握する。希望する月額上限は確認中で、現時点では金額を固定しない。
...
| API 入力のサイズ・型・回数・予算の検証が見当たらない                             | 有料処理の入口と各ツールで検証する。プロバイダー側の既存上限は未確認                                                                                       |
...
| 役割                       | 初期候補                                                         | 選定理由 / 制御                                                |
| -------------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------- |
| 意図解釈・ツール選択・回答 | `gpt-5.6-terra`                                                  | 会話文脈と複数ツールの扱いを評価。単純依頼では Luna と比較する |
...
- 月次・日次・1 実行の予算を設定可能にする。最終金額は希望上限の回答と実測後に決める。
- 外部呼び出し前に最大見込み額を予約し、実行後に精算する。並列処理と再試行も同じ台帳を通す。
- Cloud Run の複数インスタンスで共有できるトランザクション付き台帳を使用する。GCP 内の候補は Firestore。詳細な選定は既存インフラと実測費用を確認して確定する。
...
| 項目                                | 現在の扱い                       | 確定するタイミング     |
| ----------------------------------- | -------------------------------- | ---------------------- |
| 希望する月額上限と既存上限の種類    | ユーザーに確認中。金額未設定     | 有料 PoC の実行前      |
