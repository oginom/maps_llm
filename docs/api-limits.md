...
これは個人利用の誤操作・過剰取得を抑える変更。サーバー側の月次・セッション・実行ごとの予算は次節の台帳で強制する。Routes の「現在地 × 最大 5 店／比較」やチャットのツール回数制御は、機能の実装時に追加する。

## アプリ側の予算（台帳）

2026-09-21 実装。設計と運用は [budget-ledger.md](budget-ledger.md)。4 つの API route は外部呼び出しの前に台帳へ予約し、終了後に精算する。Google の日次割当と OpenAI 組織の月 $10 上限はこの台帳の外側の最終防衛線として残す。

| 範囲                         | OpenAI（micro-USD） | Places 検索 | Places 詳細 |
| ---------------------------- | ------------------: | ----------: | ----------: |
| 月（UTC の暦月）             |    750,000（$0.75） |       60 回 |      300 回 |
| セッション（ブラウザのタブ） |    200,000（$0.20） |       10 回 |       60 回 |
| 実行（1 回の検索＋追加取得） |     50,000（$0.05） |        2 回 |       20 回 |

OpenAI は `usage` から算出した実費（Luna: 入力 $0.20 / 1M、出力 $1.20 / 1M）を micro-USD の整数で加算し、呼び出し前の予約額は評価 $0.0028、検索準備 $0.00156。Google は回数で強制し、参考値として定価（検索 $0.032、詳細 $0.025）も月別に集計する。

| 応答                          | 意味                                                                     |
| ----------------------------- | ------------------------------------------------------------------------ |
| 429 `BUDGET_MONTH_EXCEEDED`   | 月の上限。翌月まで新規の検索・評価不可                                   |
| 429 `BUDGET_SESSION_EXCEEDED` | セッションの上限。そのタブでは以後の検索・追加取得を停止                 |
| 429 `BUDGET_RUN_EXCEEDED`     | その検索の上限。新しい検索を開始する                                     |
| 503 `BUDGET_UNAVAILABLE`      | 台帳（Firestore）に到達できない。有料呼び出しを開始しない（fail closed） |
| 400 `INVALID_ARGUMENT`        | `X-Session-Id` / `X-Run-Id` が無い、または UUID v4 でない                |

Google / OpenAI 側の割当超過（`RESOURCE_EXHAUSTED`）とはコードで区別する。上限値は `src/lib/budget/config.ts` の定数で、検証時のみ環境変数 `BUDGET_CAPS_JSON` で一部を上書きできる（適用時にログ出力）。
